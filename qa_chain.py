...
def parse_filename_metadata(filename: str) -> dict:
    ...
    # Try to extract author and year from patterns like:
    # 019_elhage2022toy_2022
...
DETAILED RESPONSE (cite as [Source 1], [Source 2], etc. ONLY):"""
    ...
    def query(self, question: str, k: int = None) -> QueryResponse:
        ...
        if not OLLAMA_AVAILABLE:
            return QueryResponse(answer="Ollama not available. Run: ollama serve", citations=[], sources=[])
    ...
    def literature_review(self, topic: str, num_sources: int = 12) -> QueryResponse:
        """Generate a detailed literature review section."""
        results = self.vector_store.search(topic, k=num_sources)
        ...
        prompt = f"""You are writing a literature review section for a Master's thesis on:
"Explainable AI for LLMs: Causally Grounded Mechanistic Interpretability and Concise Natural-Language Explanations"

CRITICAL CITATION RULES:
1. ONLY cite as [Source 1], [Source 2], etc.
2. DO NOT invent author names, paper titles, journal names, or page numbers
3. If you want to mention an author, say "the authors of [Source N]"
4. DO NOT write a "References" section - real citations will be added
...
REQUIREMENTS:
1. Write 5-6 LONG paragraphs (6-8 sentences each)
2. Each paragraph should focus on a different aspect/theme
3. CITE as [Source N] for every claim - NO OTHER FORMAT
4. Explain methods IN DETAIL - how they work mechanically
5. Compare and contrast different approaches
6. Identify gaps in the literature
7. Use formal academic writing style

TOPIC: {topic}

COMPREHENSIVE LITERATURE REVIEW (cite as [Source 1], [Source 2], etc. ONLY):"""
        ...
        try:
            ...
            for i, (text, metadata) in enumerate(contexts, 1):
                ...
                if source not in seen:
                    ...
                    year = metadata.get('year', '?')
                    title = metadata.get('title', 'Unknown')
                    real_sources += f"[Source {i}]: {author} ({year}). {title}\n"
                    seen.add(source)

            answer = answer + real_sources
